...
namespace NShapeTest {
	...
	public class Comparer {
		...
		public static void Compare(Project savedProject, Project loadedProject) {
			...
			// Compare Designs
			Comparer.Compare(((IStyleSetProvider)savedProject).StyleSet, ((IStyleSetProvider)loadedProject).StyleSet, version);
			CompareObjectCount(savedRepository.GetDesigns(), loadedRepository.GetDesigns());
			Comparer.Compare(savedProject.Design, loadedProject.Design, version);
			foreach (Design savedDesign in savedRepository.GetDesigns()) {
				...
				Comparer.Compare(savedDesign, loadedRepository.GetDesign(((IEntity)savedDesign).Id), version);
			}
			...
			foreach (Template savedTemplate in savedRepository.GetTemplates())
				Comparer.Compare(savedTemplate, loadedRepository.GetTemplate(savedTemplate.Id), version);
			...
			foreach (Diagram savedDiagram in savedRepository.GetDiagrams())
				Comparer.Compare(savedDiagram, loadedRepository.GetDiagram(((IEntity)savedDiagram).Id), version);
		}
		...
		public static void Compare(IStyle savedStyle, IStyle loadedStyle, int version) {
			if (savedStyle is ICapStyle && loadedStyle is ICapStyle)
				...
				Compare((IShapeStyle)savedStyle, (IShapeStyle)loadedStyle, version);
		...
		public static void Compare(IShapeStyle savedStyle, IShapeStyle loadedStyle, int version) {
			CompareBaseStyle(savedStyle, loadedStyle, version);
			Compare(savedStyle.ShadowColor, loadedStyle.ShadowColor, version);
